<template>
  <div class="centered-container">
    <h1 class="title">专利下载</h1>
    <div class="input-container">
      <n-input v-model:value="keyword" placeholder="输入关键字" class="keyword-input" />
      <n-button type="primary" class="download-button" @click="fetchData">下载专利</n-button>
    </div>
...
export default {
  ...
  methods: {
    async fetchData() {
      ...
      this.message = `正在下载与"${this.keyword}"相关的专利...请稍后......`
      try {
        await api.fetchWIPOData(this.keyword)
        this.message = `下载完成！`
      } catch (error) {
